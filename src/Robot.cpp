...
Motor Robot::LeftBackWheel(4);
Motor Robot::RightBackWheel(3);
Motor Robot::LeftFrontWheel(18);
Motor Robot::RightFrontWheel(2);
Motor Robot::LeftMiddleWheel(16);
Motor Robot::RightMiddleWheel(17);
Motor Robot::Intake(1);
Motor Robot::Catapult(10);
...
ADIDigitalIn Robot::limit_switch('A');
...
void Robot::windUp(void *ptr) {
  while(true) {
    Robot::Catapult = 127;
    if(Robot::limit_switch.get_value()) {
      Robot::Catapult = 0;
      Robot::end_task("windUp");
    }
  }
}

void Robot::rel(void *ptr) {
  while(true) {
   Robot::Catapult = 127;
   delay(200);
   Robot::Catapult = 0;
   delay(200);
   Robot::start_task("windUp", windUp);
   Robot::end_task("release");
 }
}
...
void Robot::driveControl(void *ptr)
{
  ...
  while(true)
  {
    ...
    int power = Robot::Controller1.get_analog(ANALOG_RIGHT_X);
    int turn = Robot::Controller1.get_analog(ANALOG_LEFT_Y);

    Robot::LeftFrontWheel = ((power + turn) > 127) ? 127 : (power + turn);
    Robot::LeftMiddleWheel = ((power + turn) > 127) ? 127 : (power + turn);
    Robot::LeftBackWheel = ((power + turn) > 127) ? 127 : (power + turn);
    Robot::RightFrontWheel = ((power - turn) > 127) ? 127 : (power - turn);
    Robot::RightMiddleWheel = ((power - turn) > 127) ? 127 : (power - turn);
    Robot::RightBackWheel = ((power - turn) > 127) ? 127 : (power - turn);
    ...
    if(Robot::Controller1.get_digital(DIGITAL_A)) {
      if(!Robot::task_exists("release")) {
         Robot::start_task("release", rel);
      }
    }
